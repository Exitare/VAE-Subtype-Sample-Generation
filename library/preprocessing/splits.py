...
class SplitHandler:
    ...
    @staticmethod
    def create_train_val_split(input_data: pd.DataFrame) -> Tuple:
        """
        Creates train val test split of the data provided
        @param input_data: The input data which should be split
        @param without_val: If true only a test and a train set will be created
        ...
        X_train, X_val = train_test_split(input_data, test_size=0.1, random_state=1, shuffle=True)
        return pd.DataFrame(X_train, columns=input_data.columns), pd.DataFrame(X_val, columns=input_data.columns)
